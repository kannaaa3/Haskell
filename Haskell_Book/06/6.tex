...
\begin{document}
   ...
   When we evaluate a polymorphic value, the polymorphism must be resolved to a
   specific concrete type.

   \begin{Def}[Typeclass inheritance]
       Typeclass inheritance is when a typeclasses has a superclass.
       A typeclasses requires another typeclasses to be available for a given
       type before you can write an instance.
   \end{Def}

   \begin{Def}[Effects]
       Effects are how we refer to \textit{observable} action programs may take than compute a value.
   \end{Def}

   \begin{Def}[Instance]
       An instance is the definition of how a typeclass should work for a given type.
   \end{Def}
